...
int main()
{
    ...
    for(i=0; i<infoHeader.biSizeImage; i+=3)//픽셀 데이터 중 Red/Green 비트 인덱스 0으로 만들기(Blue만 유지)
    {
        pO[i+1]=0;
        pO[i+2]=0;
    }

    fpOut = fopen("output_blue.bmp", "wb");//출력 파일 열기(바이너리 쓰기)
    ...
    fwrite(pO, sizeof(unsigned char), infoHeader.biSizeImage, fpOut);//읽어온 실제 픽셀 데이터 쓰기 
